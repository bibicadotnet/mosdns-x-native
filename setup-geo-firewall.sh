...
build_input_chain() {
    ...
    if [[ "$allowlist_count" -gt 0 ]]; then
        ...
        for port in "${ALLOW_TCP_PORTS[@]}" "${NO_RATELIMIT_TCP_PORTS[@]}"; do
...
#!/bin/bash
# geo-firewall.sh — auto-generated by setup
...
echo "  TCP no-RL  : ${NO_RATELIMIT_TCP_PORTS[*]}"
